...
<form method=post id=product enctype="multipart/form-data">
	...
	<div id="column_left">
		...
		<div class=block>
			{if $license->valid}
				<h2 style='color:green;'>Лицензия
					действительна {if $license->expiration != '*'}до {$license->expiration}{/if} для
					домен{$license->domains|count|plural:'а':'ов'} {foreach $license->domains as $d}{$d}{if !$d@last}, {/if}{/foreach}</h2>
			{else}
	...
	<div id="column_right">
		<div class=block>
			...
			<textarea style='width:420px; height:250px;' readonly>Настоящее пользовательское соглашение (далее — Соглашение) является юридическим соглашением между Пользователем системы управления сайтами «Simpla» (далее — Продуктом) и Пикусовым Д. С. (далее — Автором).
